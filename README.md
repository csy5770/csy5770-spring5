...
#### 20210601(화) 작업.
- 에러 : 404(file not found 경로 이상 에러), 500(자바 프로그램 에러)
- 프로젝트 버전을 올립니다. -> 외부 라이브러리 부분의 버전을 올림 - 메이븐 컴파일러 -> pom.xml 버전을 관리
-Controller 클래스에서 생성한 변수 사용(자바) model 객체전송(스프링) return home -> home.jsp 출력
- 출력하는 방식1 : 25년 전부터 사용하던 방식 EL(Express Language)방식출력-${변수}, <%=${변수}$>
