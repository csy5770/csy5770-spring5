...
/**
 * 이 클래스는 Admin관리자단을 접근하는 클래스 <- 디스패처 서블렛 기능을 함.
 * 디스패처 서블렛 클래스는 톰캣이 실행(web.xml) 될 때 제일 먼저 실행되는 클래스, 게이트웨이라고도 함
 * 디스패처 서블릿 실행될 때 컨트롤러의 Request 매핑경로를 재등록 함.
...
public class AdminController {
	...
	//jsp에서 게시판생성관리에 Get/Post 접근할때 URL을 bbs_type로 지정합니다.
		...
		public String selectBoardTypeList(Model model) throws Exception {//목록폼1
			...
			return "admin/bbs_type/bbs_type_list";//상대 경로일 때는 views폴더가 root(최상위폴더)
		}
		//bbs_type_list.jsp에서 게시판 생성 버튼을 클릭했을 때 이동하는 폼 경로
		...
		//bbs_type_insert.jsp의 입력폼에서 BoardTypeVO 담아서 {구현} 단, 자동으로 값이 바인딩되려면, 폼 name과 VO 멤버 변수명이 동일해야함.
		...
		public String insertBoardType(BoardTypeVO boardTypeVO) throws Exception {//입력처리1
			...
			return "redirect:/admin/bbs_type/bbs_type_list";//리다이렉트(뒤로가기불가)는 절대경로, forward:이동이 가능(뒤로가기 기능)
		}
	...
	public String insertMember(PageVO pageVO,MemberVO memberVO) throws Exception {
		...
		String rawPassword = memberVO.getUser_pw();//원시 패스워드 값
		String encPassword = passwordEncoder.encode(rawPassword);
		memberVO.setUser_pw(encPassword);
		memberService.insertMember(memberVO);
		return "redirect:/admin/member/member_list";//.jsp 생략
	}
