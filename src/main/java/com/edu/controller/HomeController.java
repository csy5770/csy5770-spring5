...
public class HomeController {
	...
	public String board_update_form(@RequestParam("bno")Integer bno,@ModelAttribute("pageVO")PageVO pageVO,Model model) throws Exception {
		//1개의 레코드만 서비스로 호출 해 모델로 전송, 첨부파일은 세로데이터를 가로데이터로 변경 후 전송.
		BoardVO boardVO = new BoardVO();
		boardVO = boardService.readBoard(bno);
		//save_file_names, real_file_names 가상 필드 값을 채움
		...
		for(AttachVO file:fileList) {//가로데이터 변경 로직
		...
		return "home/board/board_update";//.jsp 생략, 반환 값은 뷰로 보여줄 파일명
	}
