...
#### 20210602(수) 작업.
- Controller 클래스 + home/index.jsp, 그래서 컨트롤러 클래스에서 만든 변수를 index.jsp에서 사용 가능.
- 안드로이드앱 = 액티비티(java) + 레이아웃.xml(화면) 한쌍
- C# 닷넷 = .cs(프로그램) + test.aspx(화면)
- 일반 홈페이지(cafe24)-URL 직접 접근 가능(보안 위험) 
- MVC 웹 프로그램 - URL직접 접근 가능 X(보안 위험 낮음)
